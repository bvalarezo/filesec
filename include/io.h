...
/* Performs a full send to a file up to the count. It will partially send if need be*/
int full_sendfile(int out_fd, int in_fd, off_t *offset, size_t count);
/* Performs a full read+write to a file up to the count. It will partially send if need be*/
